...
void_t::mov(const mem8si_t& dst, const reg8_t& src) {
#if ARCH_LP64
    if (src.m_regcode == RAX && dst.direct64()) {
                DB(0xA2); DQ(dst.m_disp);
    } else {
...
void_t::mov(const mem32 mem32si_t& dst, const reg32_t& src) {
#if ARCH_LP64
    REX(RXB(dst));
    DB(0x89); MOD(src, dst);
#else
...
void_t nanoasm::mov(const mem32si_t& dst, const reg32_t& src) {
#if ARCH_LP64
    REX(RXB(dst));
    DB(0x89); MOD(src, dst);
#else
    if egcode == RAX && dst.direct32()) {
        DB(0xA3); DD(dst.m_disp);
    } else {
        DB(0x89); MOD(src, dst);
    }
...
#if ARCH_LP64
    if (dst.m_regcode == RAX && src.direct64()) {X(W + RXB(src));
...
void nanoas8_t& dst, const mem8si_t& src) {
#if ARCH_LP64
    if (dst.m_regcode == RAX && src.direct64()) {X(W + RXB(src));
        DB(0xA0); DQ(src.m_disp);
    } else {
...
void nanoasm::mov(const mem32_t& dst, int32_t imm32) {
    REX(RXB(dst));
    DB(0xC7); MOD(0, dst); DD(imm32)_t::mov(const mem32si:mov(const mem32_t& dst, int32_t imm32) {
    REX(RXB(dst));
    DB(0xC7); MOD(0, dst); DD(imm32)_t;
}
...
void nanoasm::movzx(const reg32_t& dst, const reg8_t& src) {
    DB(0x0F); DB(0xB6); MOD(dst.m_regcode, src.m_regcode)_t::movzid nanoasm::movsx(const reg32_t& dst, const mem8_t& src) {
    REX(RXB(src));
    6); MOD(dst, src);
}
void nanoasm_t::movzid nanoasm::movsx(const reg32_t& dst, const mem8si_t& src) {
    REX(RXB(src));
    6); MOD(dst, src);
}
void nanoasm_t::movzid nanoasm::movsx(const reg64_t& dst, const reg8_t& src) {
    REX(W + RXB(dst.m_regcode, src.m_regcode));
    6); MOD(dst.m_regcode, src.m_regcode);
}
void nanoasm_t;
}
void nanoasm::movzx(const reg64_t& dst, const mem8_t& src) {
    REX(W + RXB(dst, src));
    DB(0x0F); DB(0xB6); MOD(dst, src)_t;
}
...
void nanoasm::movsx(const reg32_t& dst, const reg8_t& src) {
    DB(0x0F); DB(0xBE); MOD(dst.m_regcode, src.m_regcode)_t;
}
void nanoasm::movsx(const reg32_t& dst, const mem8_t& src) {
    REX(RXB(src));
    DB(0x0F); DB(0xBE); MOD(dst, src)_t;
}
...
void nanoasm::movsx(const reg64_t& dst, const reg8_t& src) {
    REX(W + RXB(dst.m_regcode, src.m_regcode));
    DB(0x0F); DB(0xBE); MOD(dst.m_regcode, src.m_regcode)_t::movsid nanoasm::movzx(const reg64_t& dst, const mem8_t& src) {
    REX(W + RXB(dst, src));
    E); MOD(dst, src);
}
void nanoasm_t::movsid nanoasm::movzx(const reg64_t& dst, const mem8si_t& src) {
    REX(W + RXB(dst, src));
    E); MOD(dst, src);
}
void nanoasm_t;
...
void nanoasm::movsxd(const reg64_t& dst, const reg32_t& src) {
    REX(W + RXB(dst.m_regcode, src.m_regcode));
    DB(0x63); MOD(dst.m_regcode, src.m_regcode)_t;
}
void nanoasm::movsxd(const reg64_t& dst, const mem32_t& src) {
    REX(W + RXB(dst, src));
    DB(0x63); MOD(dst, src)_t;
}
...
dst,     REX(W + RXB(src));
    DB(0x8D); MOD(dst, src)_t::lead nanoasm::movzx(const reg64_t& dst, const mem8si_t& src) {
    REX(W + RXB(dst, 8D); MOD(dst, src);
}
void nanoasm_t;
}
void nanoasm::lea(const reg64_t& _t& src) {
    REX(W + RXB(dst,     REX(W + RXB(src));
    DB(0x8D); MOD(dst, src)_t::lea nanoasm::movsxd(const reg64_t& dst, const mem32si_t& src) {
    REX(W + RXB(dst, 8D); MOD(dst, src);
}
void nanoasm_t;
}
void nanoasm::lea(const reg64_t64_t& src) {
    REX(W + RXB(dst,     REX(W + RXB(src));
    DB(0x8D); MOD(dst, src)_t;
}
void nanoasm::lea(const reg64_t64si_t& src) {
    REX(W + RXB(dst,     REX(W + RXB(src));
    DB(0x8D); MOD(dst, src)_t;
}
...
#e_t::lea(const reg32_t& dst, const mem8_t& src) {
    DB(0x8D); MOD(dst, src);
}
void nanoasm_t;
}
void nanoasm::lea(const reg32_t& dst, const mem8si_t& src) {
    DB(0x8D); MOD(dst, src)_t::lea(const reg32_t& dst, const mem32_t& src) {
    DB(0x8D); MOD(dst, src);
}
void nanoasm_t;
}
void nanoasm::lea(const reg32_t& dst, const mem32si_t& src) {
    DB(0x8D); MOD(dst, src)_t::lea(const reg32_t& dst, const mem64_t& src) {
    DB(0x8D); MOD(dst, src);
}
...
void nanoasm::push(const reg64_t& src) {
    REX(RXB(src));
    DB(0x50 + (src.m_regcode & 7))_t::push(const mem64sh(const mem64si_t& src) {
    REX(RXB(src));
    DB(0xFF); MOD(6, src)_t;
}
void nanoasm::push(const mem64si_t& src) {
    REX(RXB(src));
    DB(0xFF); MOD(6, src)_t::pop(const reg64_t& dst) {
    REX(RXB(dst));
...
void nanoasm::jmp_short(const symbol_t& target) {
    if (INREL8(target)) {
        ...
        DB(0xEB); REL8(target);
    _t::jmpnoasm::jmp_short(const symbol_t& target) {
    if (INREL8(target)) {
        DB(0xEB); REL8(target);
    } else {
        DB(0xE9); REL32(target);
...
#if ARCH_L_tP64
void nanoasm::jmp(const reg6REX(RXB(dst));
    DB(0xFF); MOD(4, dst);
}
...
void nanoasm_t::jmp(const mem64si_t& dst) {
    REX(RXB(dst));em64si_t& dst) {
    DB(0xFF); MOD(4, dst);
}
#e_t::jmp(const reg32_t& dst) {
    DB(0xFF); MOD(4, dst);
}
...
#if ARCH_L_t::call(const reg64_t& dst) {
    REX(RXB(dst));
    DB(0xFF); MOD(2, dst);
}
